  ...
  commands: [
    createCommand(configCommand, (context) => {
      ...
      const human = [
        ...
        `user-global: ${result.layers?.userGlobalFound === true ? "found" : "not found"}`,
        `repository: ${result.layers?.repositoryFound === true ? "found" : "not found"}`,
        `machine-local: ${result.layers?.machineLocalFound === true ? "found" : "not found"}`,
        "",
    ...
    createCommand(statusCommand, async (context) => {
      ...
      if (!report.config.valid || !report.workspaceMode.valid) {
        ...
        const error = createCliError({
          ...
          suggestedNextAction: firstError.code === "status-config-invalid"
            ...
            : firstError.code === "status-workspace-mode-invalid"
              ? "Correct .qube/mode.json. Then run aiu status again."
            : "Fix the reported trusted command error. Then run aiu status again.",
          category: firstError.code === "status-config-invalid" || firstError.code === "status-workspace-mode-invalid" ? "validation" : "unexpected",
        });
        return {
    ...
    createSchemaCommand({
      ...
      sections: {
        config: {
          ...
          policyFields: ["postIssueScope", "hosts.enabled", "hosts.capabilities", "hosts.modes", "hosts.stopHookBlocking", "continuation.modes", "continuation.nativeLoopLimit", "planning.enabled", "quality.enabled", "whip.enabled", "timeouts.hookMs", "cooldowns.promptMs", "paths.stateDir", "paths.lockDir", "paths.logDir"],
        ...
        hookStop: {
          ...
          stableErrorKinds: [
            ...
            "continuation-lock-unavailable",
            "lock-held",
            "continuation-state-invalid",
            "continuation-state-unavailable",
            "continuation-state-write-failed",
            "stop-hook-already-active",
            "prompt-owned-by-other-session",
            "wait-cooldown-active",
            "duplicate-prompt-fingerprint",
            "duplicate-prompt-target",
            "duplicate-session-target",
            "native-loop-limit-exhausted",
            "hook-deadline-exhausted",
          ],
        },
        continuationState: {
          schemaVersion: 2,
          ...
          logFields: ["event", "observedAt", "eventType", "hostId", "sessionId", "selectedSessionId", "decisionId", "decisionKind", "mode", "promptKind", "selectedItem", "reasonCodes", "promptFingerprint", "targetSessionId", "deliveryState", "nativeLoopCount", "sourceSummaries", "adapterErrors", "suppressions", "elapsedMs", "message"],
          rotation: {
            maxLogBytes: 65536,
