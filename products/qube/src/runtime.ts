...
function explicitGuidedReviewError(flags: Readonly<Record<string, unknown>>, inherited: QubeInitConfig): string | undefined {
  ...
  if (getAgentHostCapabilityProfile(harness as AgentHostId).capabilities["review-isolated"].support === "unsupported") {
    return `${getAgentHostProfileSync(harness as AgentHostId).displayName} isolated review is not available yet. Use native host review with --review-mode host.`;
  }
  return undefined;
}
...
function createGuidedInitCapabilities(input: {
  ...
  const agentHarnesses: readonly GuidedHarnessChoice[] = Object.freeze(executorHostSurfaces.map(option => {
    ...
    return Object.freeze({
      ...
      ...(declared.capabilities["review-isolated"].support === "unsupported"
        ? { separateReviewUnavailableReason: `${profile.displayName} isolated review is not available yet. Use native host review with --review-mode host.` }
        : {}),
  ...
  const automatedChecks: readonly GuidedInitChoice[] = Object.freeze(executorCiProviders.map(option => Object.freeze({
    value: option.id,
    label: installOptionLabels[option.id] ?? option.id,
    description: option.id === "jenkins"
      ? "Jenkins CI is not available in the Executor workflow yet. Use GitHub or GitLab CI."
      : `Read required check results from ${installOptionLabels[option.id] ?? option.id}.`,
    ...(option.id === "jenkins" ? { disabled: true } : {}),
    recommended: option.default,
  })));
...
async function executeQubeInit(flags: Readonly<Record<string, unknown>>, args: Readonly<Record<string, unknown>>, environment: CliEnvironment): Promise<RuntimeCommandResult> {
  ...
  if (setup.review.mode === "external" && reviewProvider !== "github") {
    ...
    else if (getAgentHostCapabilityProfile(reviewHarness as AgentHostId).capabilities["review-isolated"].support === "unsupported") reviewError = `${getAgentHostProfileSync(reviewHarness as AgentHostId).displayName} isolated review is not available yet. Use native host review with --review-mode host.`;
  }
  if (reviewError) {
    const nextAction = "Correct the Review selection, then rerun qube init.";
