...
const distEntry = join(packageRoot, 'dist', 'bin', 'qube.js');
...
export function ensureQubeDist() {
  ...
  if (!existsSync(tsc)) {
    throw new Error('QUBE dist is missing or stale and typescript is not installed. Run: pnpm --dir products/qube run build');
  }
  ...
  if (result.status !== 0) {
    throw new Error('QUBE dist is missing or stale. Run: pnpm --dir products/qube run build');
  }
}
