...
describe("qube composer CLI", () => {
  ...
  it("reports Codex host capabilities without current-session assumptions", () => {
    ...
    assert.equal(getCodexHostCapability("spawn-fresh-reviewer").support, "unsupported");
    assert.match(getCodexHostCapability("spawn-fresh-reviewer").summary, /cannot itself spawn a fresh Codex reviewer/);
    ...
    assert.ok(inspection.unsupportedCapabilities.some(capability => capability.id === "spawn-fresh-reviewer"));
  ...
  it("lists standalone components without replacing them", () => {
    ...
    assert.equal(executor.capabilities.localReview.freshContextReviewerSupport, "prompt-only");
