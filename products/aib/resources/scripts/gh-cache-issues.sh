...
#
# Fetches and caches GitHub issues for agent offline access:
# 1. List of all open issues -> .issue-cache/issues-list.json
# 2. Detailed markdown files for high priority issues -> .issue-cache/*.md
#
# This allows agents to work with current project state without internet access.
#
...
# Hardcode repository for agent environments without git context
REPO_OWNER="ZarK"
REPO_NAME="ai-ws-finetune"

echo "🗂️  GitHub Issues Cache - Fetching current project state for agents"
...
if ! gh auth status >/dev/null 2>&1; then
	if [ -z "${GH_TOKEN:-}" ]; then
		...
		echo "   For agents/automation: Set GH_TOKEN environment variable"
...
echo "📊 Creating issue summary for agents..."	

# Create a summary file for agents
cat >"${CACHE_DIR}/README.md" <<EOF
# GitHub Issues Cache

This folder contains cached GitHub issues for offline agent access.
...
echo ""
echo "🤖 Agents can now work offline with current project state"
