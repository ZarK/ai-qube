...
import { isRunResult } from "./evidence-report-guards.js";
...
function createReportEvidence(
  ...
  if (report.summary.status === "passed") {
    ...
    return {
      ...
      result: "passed",
      trust: "local-evidence",
      summary: `Quality passed with ${String(report.summary.stageCount)} stage(s) and ${String(report.summary.fileCount)} file(s).`,
      recordedAt,
      ...
      states: [
        createState({
          ...
          status: "pass",
          summary: `Quality passed with ${String(report.summary.stageCount)} stage(s).`,
          command,
        }),
      ],
    };
  ...
  return {
    ...
    result: "failed",
    trust: "local-evidence",
    summary: `Quality ${report.summary.status}; ${String(failingStages.length)} stage(s) need attention.`,
    recordedAt,
    reasonCode: "local-evidence-found",
    stale: false,
    metadata: {
    ...
    states: [
      createState({
        ...
        ...(firstStage === undefined
          ...
          : {
              selectedTarget: {
                ...
                title: `Fix Quality ${firstStage.stageId}`,
        ...
        stages,
        status: "fail",
        summary: `Quality ${report.summary.status}; rerun after fixing failing stages.`,
        command,
      }),
    ],
  };
