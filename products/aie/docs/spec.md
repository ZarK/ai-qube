...
| ID | Requirement | Status |
|----|-------------|--------|
| FR-10-001 | Executor supports optional PR review agents: GitHub Copilot, Cubic, CodeRabbitAI, and custom PR comment reviewers. | Required |
...
| FR-10-015 | `policy.reviews.models` configures review model tiers (`review`, `economy`, `synthesis`), each mapping hosts (`codex`, `grok`, `claude-code`, `opencode`) to a model id and optional reasoning effort (`low`, `medium`, `high`). Only routed hosts with fixed execution adapters can be selected by `policy.reviews.route`. | Required |
...
| FR-10-018 | `providers.review.publisher` configures a user-owned GitHub reviewer identity for provider-visible publish only: `mode` `user` (default authenticated gh user), `github-app` (app id, installation id, private key path or env name), or `token` (fine-grained token env name). Config stores secret references only; private keys and tokens are never written to repository config, prompts, evidence, issue comments, PR bodies, or generated host agent assets. | Required |
| FR-10-019 | When a distinct publisher identity is configured and is not the pull request author with review permissions, Executor publishes formal pull request review events (`APPROVE`, `REQUEST_CHANGES`, `COMMENT`). Same-author or missing-permission cases degrade to comment-state publication with an explicit JSON fallback reason. `aie pr view`, `aie pr gate`, and `aie pr review publish` JSON expose publisher mode, identity class, permission status, formal-event capability, and fallback reason without secrets. Review compute remains host-run. | Required |
| FR-10-020 | `policy.reviews.route` selects a default isolated review host and model tier, and a lane `route` may override it. Routed hosts are limited to implemented fixed adapters; model ids and effort remain sourced from `policy.reviews.models`. | Required |
| FR-10-021 | Codex and Grok routed review adapters execute the exact generated lane prompt in fresh read-only sessions with fixed argument arrays, bounded execution, no subagents, and no web access by default. Executor never installs, updates, or authenticates a model host. | Required |
| FR-10-022 | Routed review batches are failure-atomic before provider mutation: every active current-head lane must produce valid complete evidence before Executor requests or publishes provider review state. Missing hosts, authentication failures, unavailable models, timeouts, non-zero exits, malformed output, stale heads, and contract mismatches fail closed. | Required |
| FR-10-023 | Executor writes routed lane evidence and trusted host provenance itself. Provenance binds the actual host, model, effort, read-only isolation, invocation identity, PR head, prompt-stack hash, and evidence digest; model-authored provenance is never trusted. | Required |
| FR-10-024 | `policy.modelRouting` configures a model catalog and route classes for delegated coding work (`mechanical-implementation`, `exploration-investigation`, `synthesis-judgment`) with fallback chains that end at the primary model. `independent-review` references `policy.reviews.models` tiers and must not duplicate review model selection. Init flags and TTY prompts offer only installed host CLIs. Host instructions and non-primary wrapper runners render the configured routes. JSON output reports selected models and substitutions. | Required |
| FR-10-025 | Every accepted routed lane preserves a typed selected route and executed route. The route records configured or fallback source, requested, transport-resolved, and host-reported model identities, a stable safe fallback reason, substitutions, and degraded reviewer separation. Lane evidence, trusted host provenance, per-lane and round provider markers, provider readback, `aie pr view` JSON, and human provenance output use the same route contract. Route identity participates in same-head publication idempotency. Publisher identity remains separate from review compute provenance. | Required |
...
| FR-17-009 | CLI runtime handlers are thin parse/render adapters around application services. Application services compose normalized policy, core rules, and production providers, and return typed result objects for human and JSON renderers. | Required |
...
| FR-17-014 | The local repository layout provider recognizes single app service repositories from exactly one root package or build signal, reports conflicting root signals as ambiguous, and uses fixture-backed tests for detection and affected-scope behavior. | Required |
| FR-17-015 | The local repository layout provider recognizes JavaScript TypeScript workspace repositories from root package workspace declarations, `pnpm-workspace.yaml`, Turbo/Nx/Lerna metadata, and conventional app/package directories when a root package manifest proves the JavaScript package boundary. | Required |
| FR-17-016 | The local repository layout provider recognizes Python workspace monorepo repositories from root `pyproject.toml` workspace declarations, Python workspace tooling metadata, and conventional member package directories when a root Python package manifest proves the package boundary. | Required |
| FR-17-017 | The local repository layout provider recognizes Rust workspace repositories from root `Cargo.toml` workspace members, `Cargo.lock`, and conventional crate directories when a root Cargo manifest proves the workspace boundary. | Required |
| FR-17-018 | The local repository layout provider recognizes Go module workspace repositories from root `go.work` use paths and member `go.mod` files when a root Go workspace file proves the workspace boundary. | Required |
| FR-17-019 | The local repository layout provider recognizes Java/Kotlin multi-project repositories from Gradle `settings.gradle(.kts)` includes or a Maven aggregator `pom.xml` `<modules>` list, plus member `build.gradle(.kts)` or `pom.xml` files, when that root proof exists. | Required |
| FR-17-020 | The local repository layout provider recognizes .NET solution repositories from a root `.sln` or `.slnx` project list plus member `.csproj` or `.fsproj` files when that root solution proof exists. `Directory.Build.props` is a supporting marker, not proof. | Required |
| FR-17-021 | The local repository layout provider recognizes Bazel, Pants, and Buck monorepo repositories from a root `MODULE.bazel`, `WORKSPACE`, `WORKSPACE.bazel`, `pants.toml`, or `.buckconfig`, plus member directories that contain `BUILD`, `BUILD.bazel`, or `BUCK` files. Supporting files are not proof, and target graphs are not inferred from filenames. | Required |
| FR-17-022 | The local repository layout provider recognizes C/C++ CMake superbuild repositories from a root `CMakeLists.txt` that lists `add_subdirectory` members or uses FetchContent, plus member `CMakeLists.txt` files for those in-repo subdirectories. `CMakePresets.json` and toolchain files are supporting markers, not proof. | Required |
| FR-17-023 | The local repository layout provider recognizes mobile app repositories from a root Android Gradle settings file plus an Android app module, a root Xcode project or workspace, a root Podfile or mobile `Package.swift`, or an Expo/React Native config plus `android/` or `ios/`. | Required |
| FR-17-024 | The local repository layout provider recognizes infrastructure repositories from root Terraform or OpenTofu files plus in-repo modules, a root Helm `Chart.yaml`, a root Kubernetes kustomization, a root Ansible playbook or `ansible.cfg`, a root `Pulumi.yaml`, or a root CDK `cdk.json`. | Required |
| FR-17-025 | The local repository layout provider recognizes docs content repositories from a root Docusaurus, MkDocs, Hugo, Sphinx, or mdBook proof file, or from a docs-only root with `docs/` content and no other workspace proof. | Required |
| FR-17-026 | The local repository layout provider recognizes polyrepo multi-checkout workspaces from a root `.gitmodules` file that lists contained checkout directories, extra contained `.git` directories under `repos/`, `checkouts/`, `externals/`, or `modules/`, or a Gradle `includeBuild` path that contains its own `.git`. Multiple remotes alone are not proof. | Required |
| FR-17-027 | The local repository layout provider recognizes generated or vendor-heavy repositories from a contained `vendor/` or `third_party/` tree, or from more than one contained generated output tree such as `dist/` and `generated/`. A single lockfile or a single generated directory is not proof. | Required |

Single app service layout support covers repositories with exactly one root package or build signal: `package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `pom.xml`, `build.gradle`, `build.gradle.kts`, root `.csproj`, or `CMakeLists.txt`. Conflicting root signals are treated as ambiguous unless a stronger supported workspace layout is detected. `aie repo affected` maps nested source, test, manifest, docs, and workflow changes to the root app project for this layout, suggests conservative gates (`build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type), and leaves generated or vendored paths out of mutation scope.

JavaScript TypeScript workspace layout support covers repositories with a root `package.json` plus one or more workspace signals: package workspaces, `pnpm-workspace.yaml`, `turbo.json`, `nx.json`, `lerna.json`, or conventional `apps/`, `packages/`, `products/`, `adapters/`, or `plugins/` member packages. `aie repo affected` maps member source changes to the owning package project, root manifest and lockfile changes to root workspace gates, and generated or vendored paths to conservative unmapped gates. JavaScript workspace tooling metadata without a root package manifest is reported as ambiguous instead of a proven workspace.

Python workspace monorepo layout support covers repositories with a root `pyproject.toml` plus one or more authoritative workspace or member signals: `[tool.uv.workspace]` members, `[tool.pdm.workspace]` members, Python tool metadata for Poetry, Hatch, PDM, or uv, member `pyproject.toml` files under `apps/`, `packages/`, `services/`, or `libs/`, and supporting markers such as `uv.lock`, `poetry.lock`, `pdm.lock`, `tox.ini`, and `noxfile.py`. `aie repo affected` maps member source changes to the owning Python package project, root manifests and Python lock or tooling changes to root workspace gates, and generated or vendored paths to conservative unmapped gates. Python workspace tooling metadata without a root `pyproject.toml` is reported as ambiguous instead of a proven workspace.

Rust workspace layout support covers repositories with a root `Cargo.toml` plus one or more authoritative workspace or member signals: `[workspace]` members, `Cargo.lock`, and member `Cargo.toml` files under `crates/`, `apps/`, `packages/`, or `services/`. `aie repo affected` maps crate source and crate manifest changes to the owning crate project, root `Cargo.toml` and `Cargo.lock` changes to root workspace gates, and generated `target/` or vendored paths to conservative unmapped gates. Rust lockfiles or nested crates without a root `Cargo.toml` are reported as ambiguous instead of a proven workspace. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Go module workspace layout support covers repositories with a root `go.work` plus member `go.mod` files listed in `use` paths or under `modules/`, `packages/`, `services/`, or `apps/`. `aie repo affected` maps module source and `go.mod` changes to the owning module, `go.work` and `go.sum` changes to root workspace gates, and `vendor/` or generated paths to conservative unmapped gates. Nested Go modules without a root `go.work` are reported as ambiguous instead of a proven workspace. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Java/Kotlin multi-project layout support covers repositories with a root Gradle `settings.gradle` or `settings.gradle.kts`, or a root Maven aggregator `pom.xml` that lists `<modules>`, plus member `build.gradle`, `build.gradle.kts`, or `pom.xml` files under those includes or under `modules/`, `apps/`, `packages/`, `services/`, `libs/`, `app/`, or `lib/`. `aie repo affected` maps module source and member manifests to the owning module, root settings, root build, or aggregator POM changes to root workspace gates, and `.gradle/`, `build/`, `target/`, or vendored paths to conservative unmapped gates. Nested Gradle or Maven modules without root settings or aggregator modules are reported as ambiguous instead of a proven multi-project. A lone root `pom.xml` or `build.gradle(.kts)` without those proofs stays a single app service. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

.NET solution layout support covers repositories with a root `.sln` or `.slnx` that lists in-repo `.csproj` or `.fsproj` members, optionally with `Directory.Build.props`. `aie repo affected` maps project source and project files to the owning project, root solution and `Directory.Build.props` changes to root workspace gates, and `bin/`, `obj/`, or vendored paths to conservative unmapped gates. Nested .NET projects without a root solution are reported as ambiguous instead of a proven solution. A lone root `.csproj` without a solution stays a single app service. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Bazel, Pants, and Buck monorepo layout support covers repositories with a root `MODULE.bazel`, `WORKSPACE`, `WORKSPACE.bazel`, `pants.toml`, or `.buckconfig`. Member projects are package directories that contain `BUILD`, `BUILD.bazel`, or `BUCK` under Pants `[source] root_patterns` when declared, otherwise under conventional `apps/`, `packages/`, `services/`, `libs/`, `src/`, or `modules/` children. `BUILD` files, `BUCK` files, and `MODULE.bazel.lock` are supporting markers, not proof. The provider does not infer a target graph from filenames and does not invoke Bazel, Pants, or Buck. `aie repo affected` maps package source and package build files to the owning package directory, root module/workspace/pants/buckconfig changes to root workspace gates, and `bazel-bin/`, `bazel-out/`, `.pants.d/`, `buck-out/`, or vendored paths to conservative unmapped gates. Nested `BUILD` or `BUCK` packages without a root proof file are reported as ambiguous instead of a proven monorepo. A lone root `BUILD` without proof is not this layout. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

C/C++ CMake superbuild layout support covers repositories with a root `CMakeLists.txt` that names in-repo members through `add_subdirectory` or uses FetchContent. Member projects are those `add_subdirectory` directories that contain `CMakeLists.txt`. `CMakePresets.json`, `CMakeUserPresets.json`, and `toolchain.cmake` are supporting markers, not proof. FetchContent declarations are not treated as in-repo members. `aie repo affected` maps project source and member CMake files to the owning project, root `CMakeLists.txt` and preset or toolchain changes to root workspace gates, and `cmake-build-*`, `_deps/`, `build/`, or vendored paths to conservative unmapped gates. Nested CMake projects without root `add_subdirectory` or FetchContent are reported as ambiguous instead of a proven superbuild. A lone root `CMakeLists.txt` without those proofs stays a single app service. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Mobile app repo layout support covers repositories with a root Android Gradle settings file plus an Android app module, a root `.xcodeproj` or `.xcworkspace`, a root `Podfile` or mobile `Package.swift`, or an Expo/React Native `app.json` / `app.config.*` plus `android/` or `ios/`. Member projects are the in-repo `android/` and `ios/` trees and declared Android app modules. `Podfile.lock` is a supporting marker. Device and simulator gates stay unconfigured unless already present. `aie repo affected` maps platform source and manifests to the owning platform project, root mobile config and settings changes to root workspace gates, and `DerivedData/`, `.gradle/`, `build/`, or vendored paths to conservative unmapped gates. Nested Android or iOS trees without a root mobile proof are reported as ambiguous instead of a proven mobile app. A generic Java/Kotlin workspace without Android or iOS proof stays that layout. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Infrastructure repo layout support covers repositories with root Terraform or OpenTofu files plus in-repo modules, a root Helm `Chart.yaml`, a root `kustomization.yaml` or `kustomization.yml`, a root `ansible.cfg` or playbook, a root `Pulumi.yaml`, or a root `cdk.json`. Member projects are declared Terraform module directories that stay in the repository, Helm `charts/` children with `Chart.yaml`, Kustomize overlay or base directories, and nested Pulumi stacks. The provider does not invoke Terraform, Helm, kubectl, Ansible, Pulumi, or CDK. Apply and deploy remain high-risk and are not suggested. `aie repo affected` maps module or chart source to the owning project, root Terraform, Helm, Kubernetes, Pulumi, or CDK files to root workspace gates, and `.terraform/` or vendored paths to conservative unmapped gates. Nested modules or charts without a root proof file are reported as ambiguous instead of a proven infrastructure repo. A lone root `.tf` file without modules is not this layout. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Docs content repo layout support covers repositories with a root Docusaurus config, `mkdocs.yml`, Hugo config, Sphinx `conf.py`, or mdBook `book.toml`, or a docs-only root with `docs/` content and no other workspace proof. Member projects are in-repo `docs/`, `website/`, `content/`, and `book/` directories that contain Markdown or reStructuredText. The provider does not invoke those docs tools. `aie repo affected` maps docs source to the owning docs project, root docs config changes to root workspace gates, and `_build/` or vendored paths to conservative unmapped gates. Nested docs trees without a root proof file are reported as ambiguous instead of a proven docs content repo. A generic application workspace with incidental markdown stays that layout. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Polyrepo multi-checkout layout support covers repositories with a root `.gitmodules` file that lists contained checkout directories, extra contained `.git` files or directories under `repos/`, `checkouts/`, `externals/`, or `modules/`, or a Gradle `includeBuild` path that contains its own `.git`. Member projects are those contained checkout directories that stay inside the inspected root. Multiple remotes, a single git root, or a checkout-shaped directory without `.gitmodules` or extra `.git` are not this layout. A Go workspace stays that layout. A Gradle `includeBuild` path without a contained `.git` stays the Java/Kotlin layout or remains unclassified. The provider does not clone remotes, follow submodule URLs, or treat sibling checkouts outside the root as members. `aie repo affected` maps checkout source to the owning checkout project, `.gitmodules` changes to root workspace gates, and generated or vendored paths to conservative unmapped gates. Checkout-shaped trees without extra git proof are reported as ambiguous instead of a proven polyrepo. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.

Generated or vendor-heavy layout support covers repositories with a contained `vendor/` or `third_party/` tree, or more than one contained generated output tree such as `dist/` and `generated/`, when no stronger workspace layout is proven. A single lockfile or a single generated directory is not this layout. A generic workspace without those proofs stays that layout. Vendor and generated trees that leave the repository root are not proof. `aie repo affected` maps remaining source changes to the root app when present, and leaves generated or vendored paths out of mutation scope. Safe default gates remain `build`, `typecheck`, `test`, `dependency-review`, `docs`, or `ci` by path type.
