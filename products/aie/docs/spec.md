...
Python workspace monorepo layout support covers repositories with a root `pyproject.toml` plus one or more authoritative workspace or member signals: `[tool.uv.workspace]` members, `[tool.pdm.workspace]` members, member `pyproject.toml` files under `apps/`, `packages/`, `services/`, or `libs/`, and supporting markers such as `uv.lock`, `poetry.lock`, `pdm.lock`, `tox.ini`, and `noxfile.py`. `aie repo affected` maps member source changes to the owning Python package project, root manifests and Python lock or tooling changes to root workspace gates, and generated or vendored paths to conservative unmapped gates. Python workspace tooling metadata without a root `pyproject.toml` is reported as ambiguous instead of a proven workspace.
