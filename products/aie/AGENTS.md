...
- Manual UI audit is enabled when the issue touches user-facing UI; use `aie audit ui <issue>` for local evidence guidance, use `aie run start --name ui-audit -- <command>` plus one bounded `aie run wait --name ui-audit --url <url> --timeout 30` for long-running local apps, then inspect the real app, capture screenshots, and record browser-observed visual analysis.
