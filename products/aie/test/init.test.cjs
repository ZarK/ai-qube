...
describe('init service', () => {
  ...
  it('renders full always-loaded workflow instructions with host projections', async () => {
    ...
    assert.match(agents, /qube app start --name ui-audit -- <command>/);
    assert.match(agents, /qube app wait --name ui-audit --url <url> --timeout 30/);
    assert.match(agents, /qube audit ui set-run --command "<command>" --url <url>/);
    ...
    assert.match(agents, /collect `qube app status --name ui-audit` logs\/status once/);
    ...
    assert.match(command, /`qube app start --name ui-audit -- <command>`/);
    assert.match(command, /`qube app wait --name ui-audit --url <url> --timeout 30`/);
    ...
    assert.match(command, /collect `qube app status --name ui-audit` logs once/);
