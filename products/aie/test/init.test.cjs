...
describe('init service', () => {
  ...
  it('renders full always-loaded workflow instructions with host projections', async () => {
    ...
    assert.match(agents, /collect `aie run status --name ui-audit` logs\/status once/);
    ...
    assert.match(command, /`aie run start --name ui-audit -- <command>`, `aie run wait --name ui-audit --url <url> --timeout 30`, `aie run status --name ui-audit`, and `aie run stop --name ui-audit`/);
    ...
    assert.match(command, /collect `aie run status --name ui-audit` logs\/status once/);
