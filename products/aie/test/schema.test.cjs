...
describe('schema command', () => {
  ...
  it('emits toolkit schema metadata with Executor-specific config sections', () => {
    ...
    assert.deepEqual(parsed.sections.config.supportedProviders.review, ['github', 'gitlab']);
    ...
    assert.equal(flag(init, 'tool').type, 'string');
    assert.deepEqual(flag(init, 'tool').options, []);
    assert.match(flag(init, 'tool').description, /Comma-separated agent harness ids/);
    ...
    assert.equal(flag(prGate, 'local-review-prompts').type, 'boolean');
    ...
    assert.ok(!reviewGate.extensions.reviewAgentValues.includes('fallback-oracle'));
