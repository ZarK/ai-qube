...
const { computeStatusFixPlanFromWorkItems } = require('../dist/deps.js');
const { createGitHubWorkProvider } = require('../dist/providers/github/github_work_provider.js');
...
describe('GitHub work provider', () => {
  ...
  it('keeps queue and dependency computations provider-neutral after mapping', async () => {
    ...
    const plans = computeStatusFixPlanFromWorkItems(items);

    assert.deepEqual(queue.items.map(item => item.issue.number), [22, 23, 21, 30]);
    assert.deepEqual(plans.map(plan => plan.issueNumber), [30, 22, 23, 21]);
  });
