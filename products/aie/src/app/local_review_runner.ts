      ...
      if (runner === 'local-host') {
        if (route) {
          ...
          if (!routed.evidence) {
            ...
            produced.push(blockedLane(lane, 'malformed', summary, routed.reasonCode ?? 'invalid model route output', null, issueNumber, input.prNumber, input.repoRoot, input.headSha, 'local-host'));
