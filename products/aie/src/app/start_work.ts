...
export async function runStartService(options: { selection: LifecycleIssueSelection; dryRun: boolean; assign: boolean; comment: boolean; context: LifecycleServiceContext }): Promise<StartServiceResult> {
  ...
  const brief = plan.ok ? buildImplementationBrief({ issueNumber: selectedIssueNumber, title: selectedItem.title, body: selectedItem.body, config: context.config, layout: await loadLayoutSnapshot(context), repoRoot: context.cwd }) : null;
