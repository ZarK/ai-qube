...
export async function runViewService(options: { issueNumber: number; context: LifecycleServiceContext; currentBranch: string | null }): Promise<ViewServiceResult> {
  ...
  return {
    ...
    brief: buildImplementationBrief({ issueNumber, title: item.title, body: item.body, config: context.config, layout: await loadLayoutSnapshot(context), repoRoot: context.cwd }),
    warnings,
