...
function writeUsage(input: readonly string[]): number | undefined {
  ...
  const usages: Record<string, { usage: string; examples: string[]; human: string }> = {
    ...
    init: {
      ...
      examples: ['aie init . --json', 'aie init . --yes --json', 'aie init . --defaults --yes', 'aie init . --from known-good --yes', 'aie init . --review-mode isolated --yes', 'aie init . --tool all --naming-rules', 'aie init . --no-milestone-ordering --package-age-days 7'],
      human: 'Usage: aie init <target> [--tool <id[,id...]|all>] [--from <path-or-repo>] [--review-mode external|host|isolated] [--publisher user|github-app] [--work-provider github|gitlab|linear|jira] [--review-provider github|gitlab] [--ci-provider github|gitlab|jenkins] [--defaults] [--yes] [--dry-run] [--force] [--json]\nInitialize Executor config and managed instruction files in a repository. This command mutates local files.\n',
    },
    switch: {
