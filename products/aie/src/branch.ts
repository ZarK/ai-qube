...
export async function runBranchCommand(input: {
  ...
  const workProvider = await createWorkProvider(config.providers.work.kind, { exec: input.exec, cwd: input.cwd, includeAssignees: false, ...config.providers.connections[config.providers.work.kind], ...config.providers.work.connection });
