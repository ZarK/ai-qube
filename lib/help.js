...
class CustomHelp extends core_1.Help {
    ...
    async showTopicHelp(topic) {
        const { config } = this;
        const bin = config.bin;
        const spec = (0, command_metadata_1.getCommandSpec)(topic.name);
        ...
        const subs = command_metadata_1.ALL_COMMAND_IDS.filter((id) => id.startsWith(topic.name + ' ') || id === topic.name);
        const children = subs
            .map((id) => command_metadata_1.COMMAND_BY_ID.get(id))
            .filter((c) => !!c && c.id !== topic.name);
    ...
    async showCommandHelp(command) {
        ...
        const spec = (0, command_metadata_1.getCommandSpec)(command.id);
        if (!spec)
            return;
        this.log('');
        const badges = [];
        if (spec.mutates)
            badges.push('MUTATES');
        if (spec.supportsDryRun)
            badges.push('--dry-run supported');
        if (spec.supportsJson)
            badges.push('--json supported');
        if (badges.length) {
