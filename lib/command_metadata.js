...
exports.ALL_COMMAND_IDS = exports.COMMAND_BY_ID = exports.COMMANDS = exports.EXIT_CODES = void 0;
exports.getCommandSpec = getCommandSpec;
exports.suggestSimilarCommands = suggestSimilarCommands;
exports.EXIT_CODES = {
    SUCCESS: 0,
    INTERNAL_ERROR: 1,
    USER_ERROR: 2,
...
exports.COMMANDS = [
    {
        ...
        description: 'Runs a series of non-mutating checks for Node version, git repository state, gh auth, linked worktree detection, base branch freshness, config presence and validity, and other prerequisites. Use before aie start or when troubleshooting.',
        examples: [
            '$ aie doctor',
            '$ aie doctor --json',
        ],
        ...
        exitCode: exports.EXIT_CODES.SUCCESS,
    },
    {
        id: 'schema',
        summary: 'Emit machine-readable description of all commands, flags, and contracts',
        description: 'Prints the complete command registry as JSON. Agents and tools use this instead of scraping --help. Includes mutation, dry-run, JSON support, examples, and stable error kinds for every command.',
        examples: [
            '$ aie schema --json',
        ],
        mutates: false,
    ...
    },
    {
        ...
        description: 'Outputs instructions for installing tab completion for bash, zsh, or fish. Completion covers command names, subcommands, and flags. No shell profile is modified by the package.',
        examples: [
    ...
    },
    {
        ...
        description: 'Interactive (or --defaults) setup that appends Executor sections to AGENTS.md / CLAUDE.md, installs .opencode/commands/make-it-so.md when targeting OpenCode, writes aie.config.json with chosen policy, and optionally creates seed labels via aie labels setup.',
        examples: [
            '$ aie init .',
    ...
    },
    {
        ...
        description: 'Parent topic for label operations. Use aie labels setup to create or update the required P1-Critical, S-Ready, C-Tooling etc. labels. Later subcommands will support listing and syncing.',
        examples: [
            '$ aie labels setup',
            '$ aie labels --help',
        ],
    ...
    },
    {
        ...
        description: 'Idempotent: creates missing labels with correct color and description, updates existing ones that drifted, leaves unrelated labels untouched. Safe to run multiple times.',
        examples: [
            '$ aie labels setup',
    ...
    },
    {
        ...
        description: 'Verifies gh auth, ensures required labels exist, checks for existing issues and installed instructions, writes a minimal aie.config.json if missing, and reports what Bootstrap artifacts are still needed. Does not generate specs or milestones.',
        examples: [
            '$ aie repo prime',
    ...
    },
    {
        ...
        description: 'Shows ready, in-progress, and blocked issues in priority order with blockers, sequence numbers, and recommended next action. Use --json for Umpire or agent consumption.',
        examples: [
            '$ aie queue',
            '$ aie queue --json',
        ],
    ...
    },
    {
        ...
        description: 'Non-mutating. Returns the single S-InProgress issue if one exists, otherwise the highest-priority S-Ready issue whose blockers are all closed. Used by autonomous loops.',
        examples: [
            '$ aie next --json',
        ],
        mutates: false,
    ...
    },
    {
        ...
        description: 'Validates pre-start policy (no linked worktree, no blocking open PRs, base branch current), removes S-Ready/S-Blocked labels, adds S-InProgress, optionally assigns, and creates a standard "started work" comment when enabled. Requires issue number or "next".',
        examples: [
            '$ aie start next',
            '$ aie start 42',
    ...
    },
    {
        ...
        description: 'Renders a focused view of one issue including computed blocker graph and recommended Executor command to run next. Use before starting or when context switching.',
        examples: [
            '$ aie view 42',
            '$ aie view 42 --json',
        ],
    ...
    },
    {
        ...
        description: 'Moves the current S-InProgress issue back to appropriate ready/blocked state, then starts the target issue. Preserves queue integrity.',
        examples: [
            '$ aie switch 67',
    ...
    },
    {
        ...
        description: 'Must be run after the PR for the issue is merged. Closes or labels the issue complete, removes S-InProgress, reconciles S-Ready/S-Blocked on all affected issues whose blockers are now satisfied, and reports the newly ready work.',
        examples: [
            '$ aie complete 42',
    ...
    },
    {
        ...
        description: 'Topic for dependency helpers. Subcommands expose the blocker graph that drives queue ordering and label sync. aie deps fix reconciles stale S-Ready/S-Blocked labels from live GitHub state.',
        examples: [
    ...
    },
    {
        ...
        description: 'Non-mutating by default with --dry-run. Updates labels so that issues whose blockers are all closed become S-Ready, and issues with open blockers become S-Blocked. Never touches S-InProgress issues.',
        examples: [
            '$ aie deps fix --dry-run',
            '$ aie deps fix',
        ],
    ...
    },
    {
        ...
        description: 'Parent for PR-related commands. Primary subcommand is aie pr gate <number> which requests configured review agents, waits, and inspects results before merge.',
        examples: [
            '$ aie pr gate 123',
            '$ aie pr --help',
        ],
    ...
    },
    {
        ...
        description: 'Triggers the repository-configured PR review agents (Copilot, CodeRabbit, custom, etc.), waits the configured duration (default 10 min), then fetches and summarizes review comments, approvals, and requested changes. Fails the gate if material issues remain.',
        examples: [
            '$ aie pr gate 123',
    ...
    },
    {
        ...
        description: 'Detects legacy gh-*.sh helpers, old AGENTS.md Executor blocks, and .opencode/commands that call the old scripts. Offers dry-run plan, compatibility wrappers, or full replacement with package-backed aie commands and instructions.',
        examples: [
...
exports.COMMAND_BY_ID = new Map(exports.COMMANDS.map((c) => [c.id, c]));
/** All command ids (for suggestions, schema, completion) */
exports.ALL_COMMAND_IDS = exports.COMMANDS.map((c) => c.id);
/**
 * Return the spec for a command id, or undefined if unknown.
 */
function getCommandSpec(id) {
    return exports.COMMAND_BY_ID.get(id);
}
/**
 ...
 */
function suggestSimilarCommands(unknownId, limit = 3) {
    const input = unknownId.toLowerCase();
    const scored = [];
    for (const id of exports.ALL_COMMAND_IDS) {
...
function levenshtein(a, b) {
    ...
    for (let j = 1; j < m; j++) {
        for (let i = 1; i < n; i++) {
            ...
            const idx = j * n + i;
            dp[idx] = Math.min(dp[j * n + (i - 1)] + 1, dp[(j - 1) * n + i] + 1, dp[(j - 1) * n + (i - 1)] + cost);
        }
    }
    return dp[m * n - 1];
}
//# sourceMappingURL=command_metadata.js.map