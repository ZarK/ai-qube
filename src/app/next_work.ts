...
export async function runNextWorkService(context: LifecycleServiceContext): Promise<NextWorkServiceResult> {
  ...
  if (active.length > 1) {
    return { workItem: null, reason: `Multiple S-InProgress issues detected (${active.length}). This is an actionable problem — run \`aie deps fix --dry-run\` then \`aie deps fix\` (or fix labels) before any new or resumed work. Selection fails.`, multipleInProgress: true, driftCount: selection.driftCount };
  }
