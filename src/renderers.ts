...
function renderDraftBody(draft: WorkItemDraft, githubBlockers: readonly number[] = []): string {
  const blockers = githubBlockers.length > 0
    ? githubBlockers.map((blocker) => `Blocked by: #${blocker}`)
