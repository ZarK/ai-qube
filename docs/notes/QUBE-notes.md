...
| Bazel/Pants/Buck monorepo | Root `MODULE.bazel`, `WORKSPACE`, `WORKSPACE.bazel`, `pants.toml`, or `.buckconfig`. Supporting `BUILD`/`BUILD.bazel`/`BUCK` package files. | Map changed paths to BUILD/BUCK package directories. Do not infer the target graph from filenames. Safe default gates stay package-directory scoped. |
| C/C++ CMake superbuild | Root `CMakeLists.txt` with `add_subdirectory` or FetchContent. Supporting `CMakePresets.json` and toolchain files. | Map changed paths to listed CMake project directories. Do not treat FetchContent as in-repo members. Avoid broad configure/build unless configured. |
