...
| AIE | `~/.qube/verification/<repository>/<issue>/` | user-local UI audit evidence | no | Default local screenshots, `browser-observation.md`, `notes.md`, and head stamp. Not committed. `policy.audit.evidenceRoot` can replace the `~/.qube/verification` prefix. Leftover `~/github-verification/` trees stay on disk and remain readable. |
