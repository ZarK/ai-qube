...
| AIU | `aiu.config.json` | standalone product config | yes | AIU init and migrate are conflict-aware, dry-runnable, and preserve state unless explicit replacement is confirmed. |
| AIU | `.umpire/` | standalone product state | no | AIU init and migrate preserve `.umpire` state, locks, and logs unless cleanup is explicitly confirmed. |
