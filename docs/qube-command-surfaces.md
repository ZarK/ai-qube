...
`qube components` exposes the four package-level component CLIs only: `aib`, `aie`, `aiq`, and `aiu`. It must not imply that every standalone-only command is required for the top-level QUBE workflow.
